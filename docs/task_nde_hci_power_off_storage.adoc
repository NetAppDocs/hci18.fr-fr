...
[role="lead"]
Lorsque vous mettez hors tension les ressources de stockage pour NetApp HCI, vous devez utiliser `Shutdown` la méthode API Element pour arrêter correctement les nœuds de stockage.
...
. Vérifiez que l'appel d'API a bien été renvoyé en vérifiant que tous les nœuds de cluster de stockage sont inclus dans la `successful` section du résultat de l'API.
+
