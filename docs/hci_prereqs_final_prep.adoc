...
. Au démarrage, accédez au BIOS en appuyant sur la `Del` touche.
. Sélectionnez l'onglet IPMI.
. Sélectionnez *Configuration réseau BMC* et appuyez sur `Enter`.
. Choisissez *Oui* et appuyez sur `Enter`.
. Sélectionnez *Configuration Address Source* et appuyez sur `Enter`.
...
. Lors de la connexion, sélectionnez l'onglet *Configuration*.
. Sélectionnez *utilisateurs*.
. Sélectionnez `ADMIN` l'utilisateur et sélectionnez *Modifier l'utilisateur*.
...
. Lors de la connexion, sélectionnez l'icône de navigation dans le menu en haut à gauche de la page pour ouvrir le tiroir de la barre latérale.
