...
| OpenStack | .iso 
|===
* (Nœuds de gestion 12.0 et 12.2 avec serveur proxy) vous avez mis à jour le contrôle du cloud hybride NetApp avec les services de gestion version 2.16 avant de configurer un serveur proxy.
...
Avant de suivre cette procédure, vous devez connaître link:concept_hci_volumes.html#persistent-volumes["volumes persistants"]et savoir si vous souhaitez les utiliser. Les volumes persistants sont facultatifs mais recommandés pour la restauration des données de configuration du nœud de gestion en cas de perte de machine virtuelle.
...
NOTE: Il est recommandé de créer un nouveau rôle NetApp HCC dans vCenter pour ajouter un sous-actif de contrôleur. Ce nouveau rôle NetApp HCC limite l'affichage des services de nœud de gestion aux ressources NetApp uniquement. Voir link:task_mnode_create_netapp_hcc_role_vcenter.html["Créez un rôle NetApp HCC dans vCenter"].
